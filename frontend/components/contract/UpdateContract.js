...
const UpdateContract = ({ contract, access_token }) => {
  ...
  const { clearErrors, error, loading, updated, updateContract, setUpdated } =
    ...
    useEffect(() => {
        if (contract) {
          ...
          setSecond_mint_price(contract.second_mint_price);
          setSupply(contract.supply);
          setRoyalty_rate(contract.royalty_rate);
          setWhitelist(contract.whitelist);
        }
  ...
  const submitHandler = (e) => {
    ...
    updateContract(contract.id, data, access_token);
  };
